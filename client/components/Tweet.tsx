...
const Tweet: React.FC<TweetProps> = ({ 
    ...
    return (
        <div className='border border-black rounded-lg w-[80%] mx-auto mb-5 sm:w-[100%] xs:w-[280px] '>
            ...
            <div className='flex justify-around items-center mb-3'>
                ...
                > {repost} <AiOutlineRetweet/></button>
                {
                  router.pathname === "/posts/[postId]" ? <></> : (
                    <Link href={`/posts/${tweetId}`}>
                      <button className='flex items-center gap-2'> {comments} Commentaires</button>
                    </Link>
                  )
                }
            </div>
